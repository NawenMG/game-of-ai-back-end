# app/controllers/comment_controller_graphql.rb
class CommentControllerGraphQl < ApplicationController
  def execute
    ...
    success_message = "GraphQL query executed for comments: #{query}, variables: #{variables}, operation_name: #{operation_name}"
    $kafka.deliver_message(success_message, topic: 'graphql_comments')

    # Usa la cache locale per 6 ore
    local_cache_key = "graphql/comments/#{Digest::MD5.hexdigest(query)}_#{Digest::MD5.hexdigest(variables.to_s)}"
    ...
    distributed_cache_key = "graphql/comments/distributed/#{Digest::MD5.hexdigest(query)}_#{Digest::MD5.hexdigest(variables.to_s)}"
