# app/controllers/archiviazione_simulazioni_graphql_controller.rb
class ArchiviazioneSimulazioniGraphqlController < ApplicationController
  def execute
    ...
    $kafka.deliver_message(success_message, topic: 'graphql_archiviazione_simulazioni')

    # Usa la cache locale per 6 ore
    local_cache_key = "graphql/archiviazione/#{Digest::MD5.hexdigest(query)}_#{Digest::MD5.hexdigest(variables.to_s)}"
    ...
    distributed_cache_key = "graphql/archiviazione/distributed/#{Digest::MD5.hexdigest(query)}_#{Digest::MD5.hexdigest(variables.to_s)}"
